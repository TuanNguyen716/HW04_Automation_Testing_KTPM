...
> Generated from `ai-audit-log.json` by the `ai-audit-logger` Agent Skill on 2026-09-14 15:23:41 +0700.
...
| Audit last updated | 2026-09-14 15:23:31 +0700 |
| Interactions recorded | 17 |

## 3. AI Tools Used

| AI tool | Interactions | Used for |
| --- | --- | --- |
| Claude | 16 | Agent Skill development, Automation script generation, Bug analysis, Debugging, Documentation, Test case analysis |
...
### A017 - Run the full Playwright suite on the live SUT and triage the failures

| Field | Value |
| --- | --- |
| Date / time | 2026-09-14 15:23:19 +0700 |
| AI tool | Claude |
| Category | Debugging |
| Feature | FR-02 Login and account lockout; FR-11 Order history view (user); FR-12 Access control |
| Test case ID | _Not provided_ |
| File modified | _Not provided_ |
| Commit | _Not provided_ |
| Output accepted | No |
...
```
Hãy giúp tôi chạy test.
```

**AI output**

```text
[EXCERPT - commands and results are verbatim from the terminal; prose is condensed]

1. Inspected the repo, confirmed the SUT answers on http://localhost:5173 (HTTP 200).
2. Ran: npx playwright test --reporter=list
   Result: 53 passed, 58 failed (37 tests x 3 browser projects).
3. All 36 firefox-project failures share one cause, not a product defect:
   Error: browserType.launch: Failed to launch the browser process.
   [err] *** You are running in headless mode.
   [err] Could not find profile folder.
   Ran: npx playwright install --force firefox (exit code 0). NOT yet re-verified - the suite has not been re-run on firefox since the reinstall.
4. The remaining 22 failures are 11 distinct cases failing identically on chrome and edge:
   FR02-TC02  email field does not block a malformed address (validity.valid expected true, got false path)
   FR02-TC05  2 consecutive wrong passwords already return 403 (expected 200 - spec locks at 3)
   FR02-TC08  login still 403 after the 30s lock window should have expired (expected 200)
   FR02-TC11  email input has type="text", spec requires type="email"
   FR02-TC12  login response body contains the plaintext password field
   FR11-TC10  GET /api/orders/391 with another user's token returned 200 with that user's order (IDOR)
   FR12-TC03  a non-admin JWT placed in localStorage unlocks the Admin UI (client-side-only guard)
   FR12-TC07  GET /api/admin/users as a normal user returned 200
   FR12-TC09  POST /api/products with no token returned 200
   FR12-TC10  POST /api/products as a normal user returned 200
   FR12-TC12  PUT /api/users/me let a user set its own role to "admin" (responded 200)
5. Assessment given to the user: items in (4) look like genuine SUT defects, except FR02-TC05 and FR02-TC08, which the AI had not yet distinguished between a real lockout-threshold/expiry defect and a test-side problem.
6. Proposed next step: an ad-hoc curl probe against /api/register and /api/login to confirm the lockout threshold. The user rejected this tool call before it ran.
```
...
Rejected the AI's curl probe of the lockout threshold before it executed, and asked for this conversation to be audited instead.

**Issues found in this output:** FR02-TC05 and FR02-TC08 were reported as failures without th (medium), The suite was run across all three browser projects even tho (low)
...
Interactions where AI output was accepted unchanged: 7 of 17.
...
| A017 | FR02-TC05 and FR02-TC08 were reported as failures without the AI being able to say whethe… | medium | The two cases depend on backend lockout state (threshold and 30s expiry) that the asserti… | Not applied - the user stopped the probe; the question is still open. | Open. |
| A017 | The suite was run across all three browser projects even though A008 already records Play… | low | The AI did not consult the existing audit log before running, so it rediscovered a known-… | Not applied by the human. The AI ran 'npx playwright install --force firefox' (exit 0) in… | Unverified - re-run the firefox project to confirm. |

Issues by severity: high=3, low=8, medium=8
...
| AI outputs rejected | 2 |
| Issues found during human review | 19 |
...
Entry A017 covers the 2026-09-14 full-suite execution session. The prompt is verbatim; the ai_output is marked [EXCERPT] - the commands and assertion messages in it are copied from the terminal, the surrounding prose is condensed. No human review of the triage has been recorded, so A017 stays pending_review. The user stopped the AI's live curl probe of the lockout threshold, so FR02-TC05 (locked after 2 failures instead of 3) and FR02-TC08 (still locked after the 30s window) remain unclassified between product defect and test defect. Playwright's Firefox was reinstalled in-session (exit 0) but the suite has not been re-run on firefox, so A008 is not yet resolved.
